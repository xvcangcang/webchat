...
覆盖场景：初始化登录流程、身份码占用换码重试、`addContact` 错误码映射（23503/23505/42501）、发送申请不自动建会话、接受申请（置 accepted + 自动建会话 + 成员写入顺序）、申请列表渲染与徽标、轮询变更检测（无会话时也同步申请状态）、`avatar_color` XSS 白名单拦截、通知默认不预览消息正文。
